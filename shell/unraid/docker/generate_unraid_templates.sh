...
<Container version="2">
  <Name>${name}</Name>
  <Repository>mddewittjr/${repo}</Repository>
  <BaseImage>Arch Linux</BaseImage>
  <Registry>https://registry.hub.docker.com/u/mddewittjr/${repo}/</Registry>
  <Project>${project}</Project>
  <Network>${mode}</Network>
  <Privileged>${privileged}</Privileged>
  <Support>${support}</Support>
  <ReadMe>https://github.com/mddewittjr/documentation</ReadMe>
  <Overview>${description}</Overview>
  <Category>${category}</Category>
  <WebUI>${webui}</WebUI>
  <TemplateURL>https://github.com/binhex/docker-templates/tree/master/mddewittjr</TemplateURL>
  <Icon>https://raw.githubusercontent.com/binhex/docker-templates/master/mddewittjr/images/${name}-icon.png</Icon>
   <ExtraParams/>
